...
```
# Overview
Single cell analysis is a rapidly growing field in biology. The ability to analyze individual cells has allowed researchers to gain a better understanding of the heterogeneity of cells within a population. This module will introduce the basic concepts of single cell analysis and provide an overview of a select number of tools available for single cell analysis.
...
if (length(packages_not_installed > 0)) {BiocManager::install(packages_not_installed,update=FALSE,ask=FALSE)}
```
...
Let's go ahead and subset our cell_data_set object to only those genes that are expressed in more than `r cellCutoff` cells.
...
* How many mRNAs do we *expect* are in a given eukaryotic cell?
...
system.time(annots<-SingleR(test = dat,
...
Once we've learned labels for each learned cluster, we can then add this information into the colData of our main object.
```{r annotation_assign}
