...
Lets grab the information for a given gene and organize it for plotting with
ggplot2. First let's grab the gene expression information from assay(se,'fpkm')
...
In order to transform these data into a long-form format, we need to 'melt'
geneData.
...
But in order to add all of the other useful information for each sample, we need
to retrieve it from colData. We need to `merge` the melted data with the column
data using a matching key (rownames of the melted expression values and
colData!)
