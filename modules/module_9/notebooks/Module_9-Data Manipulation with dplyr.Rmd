...
head(select(se.melt, Age))
head(select(se.melt, Age, value))
head(select(se.melt, value, Age))

head(select(se.melt, -Age))

# filter() to select rows matching a logical equation
head(filter(se.melt, Age == "P1"))
head(filter(se.melt, CellType %in% c("corticothal", "cpn")))
