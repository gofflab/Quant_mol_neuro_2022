...
clustData<-t(scale(t(assay(se.subset,'logfpkm'))))

# Create the dendrogram
dend<-hclust(dist(t(clustData)),method='complete')
plot(dend)
```
...
```{r}
ComplexHeatmap::Heatmap(clustData)
```
...
k<-5
# Cluster the genes
geneClust<-kmeans(clustData,k,iter.max=1000)
# Add the cluster assignments to the rowData of the SummarizedExperiment
rowData(se.subset)$geneCluster<-geneClust$cluster
```
...
# Summarise with dplyr (use the mean() function to calculate the average expression of each cluster after grouping)
tmp %>% 
  group_by(CellType,geneCluster) %>% 
  summarise(meanExp=mean(value), sdExp=sd(value))
```
4. What is the average expression and standard deviation of each cluster for each CellType and Age?
```{r}
# Summarise with dplyr (use the mean() function to calculate the average expression of each cluster after grouping)

```

5. Use the output of question 2.4
