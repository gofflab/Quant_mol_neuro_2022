...
# M1 people: DO NOT INSTALL WITH CONDA.
...
if [[ "$(arch)" == "arm64" ]]; then
  mkdir -p hisat2
  cd hisat2
  wget -N -q https://anaconda.org/bioconda/hisat2/2.2.1/download/osx-64/hisat2-2.2.1-h9722bc1_4.tar.bz2
  tar -xf hisat2-2.2.1-h9722bc1_4.tar.bz2
  rm hisat2-2.2.1-h9722bc1_4.tar.bz2
  echo "M1 rejoice!"
fi
```
...
```{bash, hisatalign2}
export PATH=$PATH:$(pwd)/hisat2/bin  # For M1 people.
cd data
for file in *.fq.gz
do
  name=${file%.fq.gz}
  hisat2 -x mm10/genome -U ${name}.fq.gz -S ${name}.sam
  echo ${name} completed!
done
```
